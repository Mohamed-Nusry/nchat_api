...
$router->group(
     ...
     */
         ...
         */
        $router->group(['prefix' => '', 'middleware' => ['auth:api', 'throttle']], function () use ($router) {
            ...
            $router->post('/users/search-user', 'UserController@searchUser');
            ...
            $router->get('/resend/verification', 'UserController@resendUserVerificationMail');
             ...
             $router->group(['prefix' => "chats"], function () use ($router) {
                $router->get('/all/{id}', 'ChatsController@showAllChat');
                $router->get('/{id}', 'ChatsController@showOneChat');
                $router->get('/allmessages/{id}', 'ChatsController@getSingleMessages');
                // $router->get('/allmessages/{id}', 'ChatsController@getSingleMessages');
                $router->post('/', 'ChatsController@create');
                //$router->put('/{id}', 'ChatsControlle@update');
                $router->delete('/{id}', 'ChatsController@delete');
            });

            //chatConversation
            $router->group(['prefix' => "chatconversation"], function () use ($router) {
                $router->get('/all/{id}', 'ChatConversationController@showAllChatConversation');
                $router->get('/{id}', 'ChatConversationController@showOneChatConversation');
                $router->get('/chats/authchats', 'ChatConversationController@getAuthChatConversations');
                $router->post('/', 'ChatConversationController@create'); //no nedd
                $router->post('/getuserdetails', 'ChatConversationController@getChatUserDetails');
                //$router->put('/{id}', 'ChatsControlle@update');
            ...
            $router->group(['prefix' => "friends"], function () use ($router) {
                // $router->get('/all/{id}', 'ChatConversationController@showAllChatConversation');
                $router->get('/request', 'FriendController@showFriendRequests');
                $router->post('/request', 'FriendController@create'); //no nedd
                // $router->post('/getuserdetails', 'ChatConversationController@getChatUserDetails');
                $router->put('/accept/{id}', 'FriendController@update');
                // $router->delete('/{id}', 'ChatConversationController@delete');
            });
